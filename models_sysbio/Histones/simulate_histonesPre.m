% simulate_histonesPre.m is the matlab interface to the cvodes mex
...
% USAGE:
% ======
% [...] = simulate_histonesPre(tout,theta)
% [...] = simulate_histonesPre(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_histonesPre(...)
%
% INPUTS:
% =======
...
function varargout = simulate_histonesPre(varargin)
...
if(max(options_ami.sens_ind)>46)
...
sol = ami_histonesPre(tout,theta(1:46),kappa(1:0),options_ami,plist,pbar(plist+1),xscale,init,data);
if(nargout>1)
