...
for cond = 1:numel(D)
    if numel(D(cond).t) > 1
        tsim = D(cond).t;%linspace(D(cond).t(1),D(cond).t(end));
    else
        tsim = D(cond).t;
    end
...
for cond = 1:14
    ...
    if numel(D(cond).t) > 1
        tsim = linspace(0,400);%linspace(D(cond).t(1),D(cond).t(end));
    else
        tsim = D(cond).t;
    end
    for iobs = 1:25
...
%             else
                plot(tsim,((sol_temp(cond).x(:,iobs)))','r'); hold on;
               % xlim([D(cond).t(1),D(cond).t(end)]);
          %  end
        else
            plot(tsim,log10((sol_temp(cond).x(:,iobs)))','rd'); hold on;
        end
        title(state_names{iobs})
    end
