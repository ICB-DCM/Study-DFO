...
for cond = 1:numel(D)
    ...
    options.ami.x0 = D(cond).init(xi_plot,D(cond).u);
    sol_temp(cond) = simulate_Bachmann_JAKSTAT_red(tsim,xi_plot,D(cond).u,[],options.ami);
    
end
sol = getSimulation_Bachmann_JAKSTAT_offsetscaling(xi_plot,sol_temp,D,options);
