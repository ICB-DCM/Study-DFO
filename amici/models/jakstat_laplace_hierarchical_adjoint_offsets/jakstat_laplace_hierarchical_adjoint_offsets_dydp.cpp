...
void dydp_jakstat_laplace_hierarchical_adjoint_offsets(double *dydp, const realtype t, const realtype *x, const realtype *p, const realtype *k, const realtype *h, const int ip) {
switch (ip) {
  case 4: {
  dydp[0] = -k[4]*1.0/(p[4]*p[4])*(x[1]+x[2]*2.0);
  dydp[1] = -k[5]*1.0/(p[4]*p[4])*(x[0]+x[1]+x[2]*2.0);
