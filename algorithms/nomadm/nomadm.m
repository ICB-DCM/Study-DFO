...
%*******************************************************************************
function getAbout(h,~,pversion,contributors)

msgbox({['NOMADm, version ',pversion], ...
        'Copyright (c) 2001-2008 by Mark A. Abramson','', ...
        'Special Thanks to:',                         '', ...
         contributors{:}},get(h,'Label')); %#ok
return
...
gui_var  = getappdata(gcbf,'gui_var');  %#ok
gui      = getappdata(gcbf,'gui');
...
%    .recombWeights    =     string choice of recombination weighting strategy
%*******************************************************************************
function loadCMAESOptions(~,~,loadCMAES,k)
...
%    .kernel          =   kernel function handle
%*******************************************************************************
function loadSurrogateOptions(~,~,loadSurrogate,k)
...
%*******************************************************************************
function solution_gui(~,~,k,iterate)

fig      = findobj('Tag', 'NOMADm_GUI');
% gui_func = getappdata(fig,'gui_func');
gui_var  = getappdata(fig,'gui_var');  %#ok
gui      = getappdata(fig,'gui');
...
for k = 1:maxPage
   ...
   x(k).ind = uicontrol(fig, ...
                 'String',             int2str(k), ...
                 'HorizontalAlignment','left', ...
                 'Position',           [leftmargin pos .05 .08]); %#ok
   x(k).val = uicontrol(fig, ...
                 'String',             xDisplay{k}, ...
                 'HorizontalAlignment','right', ...
                 'Position',           [leftmargin+.07 pos .25 .08]); %#ok
end
...
if nP
   ...
   for k = 1:maxPage
      ...
      p(k).ind = uicontrol(fig, ...
                  'String',             int2str(k), ...
                  'HorizontalAlignment','left', ...
                  'Position',           [catmargin pos .05 .08]); %#ok
      p(k).val = uicontrol(fig, ...
                  'String',             pDisplay{k}, ...
                  'HorizontalAlignment','right', ...
                  'Position',           [catmargin+.07 pos .25 .08]); %#ok
   end
