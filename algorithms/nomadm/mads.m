...
%*******************************************************************************
%   Copyright (c) 2001-2012 by Mark A. Abramson
%
...
try
   ...
   while ~terminate(Options.Term,RunData,Options.runStochastic)
      ...
      if Options.debug >= 2
         disp([Problem.typeProblem,':    Mesh size = ', ...
               num2str(RunData.delta,8)]);
      end

      % Begin SEARCH step
      Cache = getappdata(0,Problem.nameCache);
      if Options.debug >= 1
         disp([Problem.typeProblem,': Begin Search ',int2str(RunData.nIter+1)]);
      end
      [success,TempFilter,termflag] = search(Problem,Options,RunData, ...
                                             Cache.Filter);
      if termflag, RunData.stopRun = 1; continue, end
      if Options.debug >= 1
         disp([Problem.typeProblem,': End Search ',int2str(RunData.nIter+1)]);
      end
      Cache = getappdata(0,Problem.nameCache);
      ...
      if ~success
         ...
         if Options.debug >= 1
            disp([Problem.typeProblem,': Begin Poll ',int2str(RunData.nIter+1)]);
         end
         [success,TempFilter,RunData] = poll('P',Problem,Options,RunData,...
                                             RunData.pcenter,Cache.Filter);
         if Options.debug >= 1
            disp([Problem.typeProblem,': End Poll ',int2str(RunData.nIter+1)]);
         end
         Cache = getappdata(0,Problem.nameCache);
         Cache.Filter = TempFilter;
         setappdata(0,Problem.nameCache,Cache);
      ...
      if ~success && Problem.isMVP
         if Options.debug >= 1
            disp([Problem.typeProblem,': Begin MV Poll ',int2str(RunData.nIter+1)]);
         end
         success = mvpPoll(Problem,Options,RunData);
         if Options.debug >= 1
            disp([Problem.typeProblem,': End MV Poll ',int2str(RunData.nIter+1)]);
         end
      end

      % Update MADS parameters
      if Options.debug >= 2
         disp([Problem.typeProblem,':    Success = ',int2str(~~success)]);
      end
...
if Options.debug >= 1
   disp([Problem.typeProblem,': Begin Initial Points']);
end
...
if Options.debug >= 1
   disp([Problem.typeProblem,': End Initial Points']);
end
return
...
if strncmpi(Search.type,'SPS',3) || strncmpi(Search.type,'CUS',3)
...
if Options.debug >= 2
   ...
   disp(['   # Points:    ', num2str(Search.nPoints)]);
end
switch upper(Search.type)
...
case {'DACE','SPS-DACE'}
   ...
   if Options.debug >= 2
      disp([[msg;repmat(' ',length(theta)-1,length(msg))],num2str(theta)]);
   end
...
if strncmpi(Search.type,'SPS',3) || strcmpi(Search.type,'CUSTOMS')
   for k = 1:length(Sites)
      ...
      if Options.debug >= 2
         disp(' ');
         disp(['   True objective f-value = ', num2str(Sites(k).f)]);
         disp(['   User surrogate f-value = ', num2str(Sites(k).sf)]);
      end
      ...
      if Options.debug >= 2
         disp(['   Additive SPS f-value   = ', num2str(Sites(k).f)]);
         disp(' ');
      end
   end
end
...
for k = 1:nP
   ...
   if ~isempty(P(k).p)
      p = '';
      if Options.debug >= 3, disp('   evalPointSet:  p = '); end
      for i = 1:length(P(k).p)
         if ischar(P(k).p{i})
          ...
          if Options.debug >= 3, disp(['     ',[p{i}]]); end
      end
   end
   if Options.debug >= 3
      disp('   evalPointSet:  x = ');
      disp([repmat('     ',P(k).n,1),num2str(P(k).x)]);
   end
   ...
   if inOmega(P(k),Problem.Omega)
      ...
      if hit
         ...
         if Options.debug >= 3
            disp(['   evalPointSet:  f = ',num2str(P(k).f)]);
         end
...
nP = length(Pset);
[P,feas,infeas] = deal([]);
if Options.debug >= 2, disp(['   Points identified: ',int2str(nP)]); end
...
%   randn('state',myeval(Options.seed));
   ...
   while ~any(stopflag)
      nIter = nIter + 1;
      if Options.debug >= 2, disp(['   CMA-ES ',int2str(nIter)]); end
